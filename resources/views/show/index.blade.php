            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="sm:flex sm:items-center">
                    <div class="sm:flex-auto">
                        <h1 class="text-xl font-semibold text-gray-900">{{__('Shows')}}</h1>
                        <p class="mt-2 text-sm text-gray-700">{{__('show.description_list')}}</p>
                    </div>
                                            ...
                                            <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                                <div class="flex items-center">
                                                    <div class="h-10 w-10 flex-shrink-0">
                                                        <img class="h-10 w-10 rounded-full" src="{{$show->poster_path}}" alt="">
                                                    </div>
                                                    <div class="ml-4">
                                                        <div class="font-medium text-gray-900">{{$show->title}}</div>
                                                        <div class="text-gray-500">{{$show->original_title}}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                <!-- This example requires Tailwind CSS v2.0+ -->
                                                <form action="{{route('show.delete', $show)}}" method="POST">
                                                    @csrf <!-- {{ csrf_field() }} -->
